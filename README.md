...
1. `--no-ctoken`, `--no-ptoken`, `--no-cpos`, `--no-ppos`, `--no-cchunk` and `--no-pchunk` means no current token, no previous token, no current pos tag, no previous pos tag, no current syntactic chunk and no previous syntactic chunk as features, respectively.
...
1. `FEATURE_CODE` is a sequence of digits of length six. It's the true-false value of `--no-ctoken`, `--no-ptoken`, `--no-cpos`, `--no-ppos`, `--no-cchunk` and `--no-pchunk`. Where 1 means True and 0 means False.
